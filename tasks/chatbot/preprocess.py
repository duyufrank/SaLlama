...
class CadgeDatasetPreprocessor(object):
    def __init__(self, hparams):
        ...
        self.data_tgt_dir = Path(f"{BASE_DIR}/datasets/matinf_1.0_encrypted")
        self.model_name_or_path = self.hparams.model_name_or_path
        self.model_name_or_path = self.resource_dir.joinpath(f"external_models/{self.model_name_or_path}")
        self.local_model_dir = self.model_name_or_path
    ...
    def preprocess_cadge_data(self):
        ...
        tokenizer = LlamaTokenizer.from_pretrained(self.local_model_dir)
        #resource_file = self.data_src_dir.joinpath("resource.txt")
        #resource_dict = json.load(resource_file.open("rb"))

        print(f"original vocab size of Llama: {len(tokenizer.get_vocab())}")
...
if __name__ == '__main__':
    ...
    # 最好在命令行传入这个值
    # hparams.model_name_or_path=/data/weiguang/Llama-2-7b-chat-hf
