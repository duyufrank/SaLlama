...
#edited by Yu
def pad_all_triples(alltriples,pad_token=2):
    max_len = 6
    padded_all_triples = []
    for triples in alltriples:
        ...
        for t in triples:
            if len(t)>=max_len:
                ...
                while len(t)<max_len:
                    t.append(pad_token)
            padded_triples.append(t)
        padded_all_triples.append(padded_triples)
    return padded_all_triples
...
class CommonGraphDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, hparams, batch_size, tokenizor, embedding_size=768, data_name='train'):
        ...
        self.NO_RET_TRIPLE = [self.NO_RET_TOK_ID]*6
        self.BOS_TRIPLE = [self.TRIPLE_BOS_ID]*6
    ...
    def process_triple(self, all_triples, target_triples):
        """
        Returns:
            src_triples: for each data sample [[head_id, rel_id, rel_id, tail_id], ...]
        ...
        for triple_list in all_triples:
            ...
            for t in triple_list:
                ...
                #modified by YU
                head = [self.word2id_helper(i) for i in head]
                tail = [self.word2id_helper(i) for i in tail]
                rel = [self.word2id_helper(i) for i in rel]
                processed_triple.append([i for sublist in [head,rel,tail] for i in sublist])
        ...
        for t in target_triples:
            ...
            if set(head).issubset(set(self.word2id)) and set(tail).issubset(set(self.word2id)):
                target_head_ents.append([self.word2id[i] for i in head])
                target_tail_ents.append([self.word2id[i] for i in tail])
        target_ents = target_head_ents + target_tail_ents
        target_ents = [i for j in target_ents for i in j]
    ...
    def make_graph(self, all_triples, main_entities, target_ents):
        ...
        target_node = []
        embedding = []
        all_triples = pad_all_triples(all_triples,self.tokenizer.pad_token_id)
        ...
        if len(edges) > 0:
            ...
            g.ndata['h'] = self.NO_RET_TOK_ID * torch.ones((2, 6)).long()
    ...
    def __getitem__(self, i):
        ...
        text = '问题: ' + src_text+ ' 回答: ' + tgt_text +'</s>'
        src_text = text
        tgt_text = text
    ...
    def collate_fn(self, batch):
        x_encoding = self.tokenizer(
            ...
            truncation=True,
            padding="max_length", # modified by Xinyi
